...
namespace UnityEngine.Rendering.Universal.Internal
{
    ...
    public class PostProcessPass : ScriptableRenderPass
    {
        ...
        void DoSubpixelMorphologicalAntialiasing(ref CameraData cameraData, CommandBuffer cmd, RenderTargetIdentifier source, RenderTargetIdentifier destination)
        {
            ...
            //---
            RenderTextureDescriptor descEdgeTexture = GetCompatibleDescriptor(m_Descriptor.width, m_Descriptor.height, m_SMAAEdgeFormat, tempDepthBits);
            RenderTextureDescriptor descBlendTexture = GetCompatibleDescriptor(m_Descriptor.width, m_Descriptor.height, GraphicsFormat.R8G8B8A8_UNorm);
#if UNITY_IOS
            descEdgeTexture.memoryless = RenderTextureMemoryless.Color;
            descBlendTexture.memoryless = RenderTextureMemoryless.Color;
#endif
            cmd.GetTemporaryRT(ShaderConstants._EdgeTexture, descEdgeTexture, FilterMode.Bilinear);
            cmd.GetTemporaryRT(ShaderConstants._BlendTexture, descBlendTexture, FilterMode.Point);
            //---
            ...
            cmd.SetRenderTarget(new RenderTargetIdentifier(ShaderConstants._EdgeTexture, 0, CubemapFace.Unknown, -1),
                RenderBufferLoadAction.DontCare, RenderBufferStoreAction.DontCare/*RenderBufferStoreAction.Store*/, stencil,
                RenderBufferLoadAction.DontCare, RenderBufferStoreAction.Store);
